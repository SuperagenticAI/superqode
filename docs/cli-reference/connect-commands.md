...
| `fx-key` | fx with `AI_GATEWAY_API_KEY` (Open harnesses). Child process only; not the Vercel login. Does not open SuperQode's local picker; Fx custom connections stay in Fx settings. |
