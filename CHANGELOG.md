...
- Restored the OpenCode model picker from before 0.2.118. The loading
  frame that replaced the ACP catalog while models were fetched is
  removed.
...
- Pushing a `v*` tag runs `.github/workflows/publish.yml`: it uploads
  that tag to PyPI with `PYPI_API_TOKEN` and opens the matching GitHub
  release. It does not run on ordinary commits.
