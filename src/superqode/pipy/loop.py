...
from .tools_base import AgentTool, AgentToolResult
    ...
    *,
    ...
    while True:
        ...
        while has_more_tools or pending:
            ...
            # Pi: refuse all tool calls when the response hit the output token limit.
            if assistant.stop_reason == "length" and calls:
                for call in calls:
                    async for event in _fail_truncated_tool(call):
                        yield event
                        if isinstance(event, MessageEndEvent) and isinstance(
...
async def _fail_truncated_tool(call: ToolCall) -> AsyncIterator[AgentEvent]:
