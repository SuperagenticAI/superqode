"""
report Web Dashboard - Interactive local viewer for Validation Reports.

Provides a web-based interface for viewing report findings with:
- Severity filtering
- Interactive findings table
- Verified fixes visualization
- Trend charts from historical reports
"""
...
<head>
    ...
    <title>SuperQode report Dashboard</title>
    <style>
        :root {
            --bg-primary: #0a0a0a;
            --bg-secondary: #1a1a1a;
...
<body>
    <div class="container">
        <header class="header">
            <div class="logo">SuperQode report Dashboard</div>
            <div class="meta">
        ...
        <footer class="footer">
            Generated by <a href="https://github.com/superqode/superqode">SuperQode</a> - Agentic validation and evaluation
        </footer>
    </div>
...
def start_dashboard(
    ...
    """
    Start the report web dashboard.

    Args:
        qr_path: Path to specific report JSON file. If None, uses latest.
        project_root: Project root to search for reports. Defaults to cwd.
    ...
    if qr_path is None:
        ...
        if qr_path is None:
            raise FileNotFoundError("No report files found. Run 'superqode qe run .' first.")
    ...
    with socketserver.TCPServer(("", port), QIRDashboardHandler) as httpd:
        ...
        print(f"report Dashboard running at {url}")
...
def export_html(
    ...
    Args:
        qr_path: Path to report JSON file
