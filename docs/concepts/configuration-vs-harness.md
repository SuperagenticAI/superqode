...
| **Generated by** | `superqode config init` | `superqode harness init`, `superqode harness wizard`, `superqode local init`, or `:local init` |
| **Loaded by** | Discovered automatically from project, user, or system paths | Loaded explicitly with `--harness`, `:harness`, or `superqode harness run --spec` |
| **Required** | No. SuperQode works with zero config | No. The TUI and `--print` use built-in defaults |

`superqode.local.yaml` is a HarnessSpec too. It is generated by the local setup flow and is named differently so it is obvious that the file targets the current machine and local model stack.
