"""
Init command - Initialize validation workflow configuration for a project.
...
DEFAULT_CONFIG = """# SuperQode Configuration (used by validation workflow)
...
"""
    ...
    return f'''{framework_comment}# SuperQode Configuration (used by validation workflow)
...
def init(path: str, force: bool, minimal: bool, guided: bool):
    """Initialize validation workflow configuration for a project.

    Creates a superqode.yaml configuration file and .superqode directory.

    Examples:

        superqode init              # Initialize current directory

        superqode init ./myproject  # Initialize specific directory

        superqode init --minimal    # Create minimal config

        superqode init --guided     # Run guided setup wizard
    """
    project_root = Path(path).resolve()

    console.print()
    console.print(Panel("[bold]validation workflow Initialization[/bold]", border_style="cyan"))
    console.print()
    ...
    console.print(
        "[dim]⚡ Power validation roles: unit, integration, api, ui, accessibility, security, usability[/dim]"
    )
    console.print(
    ...
    console.print()
    console.print("[green]validation workflow initialized successfully![/green]")
    console.print()
    console.print("[dim]Next steps:[/dim]")
    console.print("  1. Review superqode.yaml and customize settings")
    console.print("  2. Run [cyan]superqode qe run . --mode quick[/cyan] for a quick scan")
    console.print("  3. Run [cyan]superqode qe run . --mode deep[/cyan] for full analysis")
    console.print()
...
def _run_guided_setup(project_root: Path, config_file: Path, superqode_dir: Path) -> int:
    ...
    console.print()
    console.print("Select your team size to optimize validation role configuration:")
    ...
    console.print(
        Panel(
            "[bold green]validation workflow initialized successfully![/bold green]",
            border_style="green",
        )
    )
    console.print()
    ...
    if Confirm.ask("Run a quick scan to validate setup?", default=True):
        ...
        try:
            result = subprocess.run(
                ["superqode", "qe", "run", str(project_root), "--mode", "quick"],
        ...
        console.print()
        console.print("[dim]Next steps:[/dim]")
        console.print("  1. Run [cyan]superqode qe run . --mode quick[/cyan] for a quick scan")
        console.print("  2. Run [cyan]superqode qe dashboard[/cyan] to view results")
        console.print()
...
def _create_superqode_directory(superqode_dir: Path) -> None:
    ...
    gitignore_content = """# SuperQode / validation workflow temporary files
temp/
*.log
...
def _create_minimal_config(team_name: str) -> str:
    ...
    return f'''# SuperQode Configuration (Minimal, used by validation workflow)
superqode:
  version: "1.0"
  team_name: "{team_name}"
