...
- **Live tool progress + runtime footer in channels** — while the agent works, the "Working on it" message is edited in place with the running tool and call count (Telegram editMessageText, Slack chat.update, Discord PATCH; throttled), and the final reply carries a compact `model · cwd` footer.
- **Hermes Agent via ACP, verified end to end** — `uv tool install 'hermes-agent[acp]'`, point Hermes at any local OpenAI-compatible server in `~/.hermes/config.yaml`, `superqode agents doctor hermes --live` confirms session + models + modes, then `:connect acp hermes` in the TUI. Catalog entry updated with the working install command and local-model configuration.
- **`superqode daemon` + chat channels** — supervise long local runs from Telegram, Slack, or Discord: plain text runs prompts (and steers a live run), tool approvals relay to chat with Approve/Always/Deny buttons on all three platforms (Telegram inline keyboards, Slack Block Kit, Discord components), completion messages carry Status/New/Stop buttons, plus `/status`, `/stop`, `/new`, `/model`, `/cd`. Slack replies are threaded under the triggering message and duplicate event deliveries (app_mention + message for the same text) are coalesced. Allowlist-first security (unknown chats only ever get pairing instructions), tokens from env (a local `./.env` is also read by the daemon), outbound-only transports (long polling, Socket Mode, Gateway with exponential backoff; no inbound HTTP). Telegram is stdlib-only; Slack/Discord need `pip install superqode[channels]`. One agent session per chat, same approval semantics as the TUI.
- **Local Agentic Coding positioning** — SuperQode now names its category: agentic software engineering on open models running on your own hardware. New pillar page (`docs/local-agentic-coding.md`) in the nav, tagline and README updated, and the term carried through the `superqode local` CLI.
- **TUI `:local` command** — the Local Stack Doctor inside the TUI: `:local` (or `:local doctor`) renders the hardware/engine/model verdict without blocking the UI; `:local packs` lists model policy packs. Autocomplete and `:help` wired.
- **Local Stack Doctor** — `superqode local doctor` detects hardware (Apple Silicon generation + unified memory, NVIDIA VRAM, M5 Neural Accelerators on macOS 26.2+), inventories seven inference engines (Ollama, LM Studio, mlx-lm, llama.cpp, vLLM, SGLang, DS4) and every downloaded model (Ollama library, HF cache, LM Studio), then recommends a tuned engine + model stack, preferring what is already installed and downloaded. `--generate harness.yaml` writes a ready-to-run harness routed to the right provider for where the model actually lives. `--json` for scripts.
- **Recommendation matrix as data** — eight hardware tiers map to ranked engines and models in a shipped `stack_matrix.yaml`; users override or extend tiers via `~/.superqode/stack_matrix.yaml` without waiting for a release.
- **Model policy packs** — shipped tuned defaults per open-model family (`gemma4`, `qwen3`, `qwen-coder`, `ds4`, `devstral`, `gpt-oss`, `glm`): temperature, prompt level, tool-call format, history budget. Auto-detected from the model id (longest match wins) or referenced explicitly with `model_policy.pack`; explicit spec fields and `model_policy.config` still win. User packs in `~/.superqode/model-packs/` override shipped ones. `superqode local packs` lists them.
- **Local bench** — `superqode local bench` streams one coding-shaped completion against running OpenAI-compatible endpoints and reports time-to-first-token (prefill) and decode tokens/second, the two numbers that decide whether a local model is pleasant in an agent loop.
- **MLX server lifecycle** — `superqode providers mlx server --model <hf-id>` starts `mlx_lm.server` on the OpenAI protocol; `superqode providers mlx doctor` checks the install and a live endpoint. Ollama 0.19+ MLX-runtime eligibility (Apple Silicon, >32GB) is detected and reported by the doctor.
- **Utility model routing** — `SUPERQODE_UTILITY_PROVIDER` redirects small quality-tolerant calls (rubric grading, auto-memory extraction) to a cheaper route: any `provider/model`, or `apple-fm` for the on-device Apple Foundation Model (zero cost, no server, via the apple-fm-sdk). Fails open to the session model.
- **`get_context_remaining`** — a read-only tool that reports the live context window, estimated usage, and remaining budget before automatic compaction, so the model can decide whether to read more material or consolidate and finish. Wired with real per-run numbers from the loop; in the coding, full, and ds4 profiles.
